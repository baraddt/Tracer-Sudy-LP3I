...
export default function User() {
    ...
    const handleSearch = async (e) => {
        ...
        if (query.trim() === '') {
            setUsersList([]); // Kosongkan hasil pencarian jika query kosong
            return;
        }

        try {
            const response = await fetch(`/api/users?search=${query}`); // Ganti dengan endpoint API kamu
            const data = await response.json();
            setUsersList(data.data); // Menyimpan hasil pencarian ke dalam state users
        } catch (error) {
    ...
    return (
        <div className="container mt-4">
            <div className="rounded bg-white p-3">
                <h4 className="mb-4 fw-semibold text-dark">Admin</h4>
