    ...
    const fetchData = async () => {
        try {
            ...
            if (storedData) {
                ...
                setProgramList(Array.isArray(parsedData.data) ? parsedData.data : [parsedData.data]); // Pastikan data dalam bentuk array
                return;
            }

            // Coba fetch data jika tidak ada di localStorage
            const response = await axiosClient.get('/prodi/all');
            console.log("Data fetched by API:", response.data);
            setProgramList(Array.isArray(response.data.data) ? response.data.data : [response.data.data]); // Pastikan data dalam bentuk array
            localStorage.setItem("prodyData", JSON.stringify(response.data)); // Menyimpan data ke localStorage
        } catch (error) {
